...
static FuType *FuParser_parse_array_slice_type(FuParser *p) {
    ...
    if (FuParser_check_token(p, TOK_SEMI)) {
        ...
        FuType *ty = FuType_new(p->ctx, sp, TY_ARRAY);
        ty->_array.ty = inner;
        ty->_array.size = size;
        return ty;
    }
    FuToken close_tok = FuParser_expect_token(p, TOK_CLOSE_BRACKET);
    FuSpan *sp = FuSpan_join(open_tok.sp, close_tok.sp);
    FuType *ty = FuType_new(p->ctx, sp, TY_SLICE);
    ty->_slice = inner;
    return ty;
}
...
FuAttr *FuParser_parse_doc_attr(FuParser *p) {
    ...
    FuStr *symbol = FuCtx_get_symbol(p->ctx, tok.sym);
...
static void FuParser_parse_fn(FuParser *p, FuVec *attrs, FuSpan **sp_p, FuIdent **ident_p, FuVec **params_p,
                              FuFnSig **sig_p, FuBlock **body_p) {
    FuSpan *lo = FuParser_current_span(p);
    ...
    *params_p = FuParser_parse_fn_params(p, TOK_CLOSE_PAREN, FU_TRUE);
    ...
    FuFnSig *sig = FuFnSig_from_params(p->ctx, NULL, *params_p, return_ty);
...
FuNode *FuParser_parse_item_interface(FuParser *p, FuVec *attrs, fu_vis_k vis) {
    ...
    FuNode *nd = FuNode_new(p->ctx, sp, ND_INTERFACE);
    nd->attrs = attrs;
